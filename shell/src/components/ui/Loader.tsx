...
export const Loader: React.FC<LoaderProps> = ({
  ...
  const bgColor = color.startsWith('text-')
    ...
    <div className={`inline-flex flex-col items-center justify-center gap-2 ${className}`}>
      ...
      {variant === 'dots' && (
        <div className="flex items-center space-x-1.5">
          <div className={`${dot} ${bgColor} rounded-full animate-bounce [animation-delay:-0.3s]`} />
          <div className={`${dot} ${bgColor} rounded-full animate-bounce [animation-delay:-0.15s]`} />
      ...
      {variant === 'pulse' && (
        <div className={`relative ${container} flex items-center justify-center`}>
          <span className={`absolute inline-flex h-full w-full rounded-full ${bgColor} opacity-75 animate-ping`} />
